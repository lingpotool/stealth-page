...
export class ElementRect {
  ...
  async location(): Promise<{ x: number; y: number }> {
    ...
    try {
      const metrics = await this._ele.session.send<{ visualViewport: { pageX: number; pageY: number } }>("Page.getLayoutMetrics");
      return {
  ...
  async viewport_location(): Promise<{ x: number; y: number }> {
    try {
      const { model } = await this._ele.session.send<{ model: { border: number[] } }>("DOM.getBoxModel", {
  ...
  async screen_location(): Promise<{ x: number; y: number }> {
    const vp = await this.viewport_location();
    try {
      const { result } = await this._ele.session.send<{ result: { value: { x: number; y: number; dpr: number } } }>("Runtime.evaluate", {
  ...
  async viewport_click_point(): Promise<{ x: number; y: number }> {
    try {
      ...
      if (backendId > 0) {
        const { model } = await this._ele.session.send<{ model: { padding: number[] } }>("DOM.getBoxModel", { backendNodeId: backendId });
