 ...
 * 页面级别的 Cookie 设置类
 */
export class PageCookiesSetter extends CookiesSetter {
  /**
   * 获取所有 cookie
   */
  async getAll(urls?: string[]): Promise<CookieData[]> {
    const params: Record<string, any> = {};
    if (urls) params.urls = urls;
    
    const { cookies } = await this._owner.cdpSession.send<{ cookies: any[] }>("Network.getAllCookies", params);
