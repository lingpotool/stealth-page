...
exports.make_input_data = make_input_data;
exports.send_key = send_key;
exports.input_text_or_keys = input_text_or_keys;
exports.Keys = {
    NULL: "\uE000",
    CANCEL: "\uE001",
    HELP: "\uE002",
    ...
    CTRL_A: [process.platform === 'darwin' ? '\uE03D' : '\uE009', 'a'],
    CTRL_C: [process.platform === 'darwin' ? '\uE03D' : '\uE009', 'c'],
    CTRL_X: [process.platform === 'darwin' ? '\uE03D' : '\uE009', 'x'],
    CTRL_V: [process.platform === 'darwin' ? '\uE03D' : '\uE009', 'v'],
    CTRL_Z: [process.platform === 'darwin' ? '\uE03D' : '\uE009', 'z'],
    CTRL_Y: [process.platform === 'darwin' ? '\uE03D' : '\uE009', 'y'],
    NUMPAD0: "\uE01A",
    NUMPAD1: "\uE01B",
    NUMPAD2: "\uE01C",
    NUMPAD3: "\uE01D",
...
function make_input_data(key, modifiers = 0, keyUp = false) {
    const data = exports.keyDefinitions[key];
    if (!data)
        return null;
    const result = { modifiers, autoRepeat: false };
...
async function send_key(page, key, modifiers = 0) {
...
async function input_text_or_keys(page, textOrKeys) {
    ...
    const { modifier, text } = keysToTyping(textOrKeys);
    if (modifier !== 0) {
