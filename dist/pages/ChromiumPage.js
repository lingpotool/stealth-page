...
class ChromiumPage extends ChromiumBase_1.ChromiumBase {
    constructor(addrOrOpts) {
        if (addrOrOpts instanceof Chromium_1.Chromium) {
            super(addrOrOpts);
        }
        else {
            super(new Chromium_1.Chromium(addrOrOpts));
        }
    }
    get tab_id() {
    ...
    async new_tab(url, options) {
        ...
        if (options?.newContext) {
            ...
            const tab = new ChromiumTab_1.ChromiumTab(this._browser, tabId);
            await tab.init();
            return tab;
        }
        const { targetId } = await this._browser.cdpSession.send("Target.createTarget", {
            url: url || "about:blank",
            newWindow: options?.newWindow,
            background: options?.background,
        });
        const tab = new ChromiumTab_1.ChromiumTab(this._browser, targetId);
        await tab.init();
        return tab;
    }
    async get_tab(options) {
        ...
        if (options?.idOrNum !== undefined) {
            if (typeof options.idOrNum === "string") {
                ...
                if (options.asId)
                    return found.id;
                const tab = new ChromiumTab_1.ChromiumTab(this._browser, found.id);
                await tab.init();
                return tab;
            }
            else {
        ...
        const tab = new ChromiumTab_1.ChromiumTab(this._browser, filtered[0].id);
        await tab.init();
        return tab;
    }
    async close() {
    ...
    async get_tabs(title, url, tabType, asId) {
    ...
    async get_tab_ids(title, url, tabType) {
    ...
    async close_tab(tabId) {
        if (tabId)
            return this._browser.close_tab(tabId);
        return this.close();
    }
    get tabs_count() {
    ...
    async latest_tab() {
        ...
        const tab = new ChromiumTab_1.ChromiumTab(this._browser, lastTab.id);
        await tab.init();
        return tab;
    }
    async close_tabs(tabIds, others = false) {
        ...
        if (others) {
            ...
            for (const id of ids)
                await this._browser.close_tab(id);
        }
    }
    async process_id() {
        try {
            const { processId } = await this._browser.cdpSession.send("SystemInfo.getProcessInfo");
            return processId;
        }
        catch {
            return null;
