...
class ElementClicker {
    ...
    async left(byJs = false, timeout = 1.5, waitStop = true) {
        ...
        try {
            ...
            const { result: tagResult } = await this._ele.session.send("Runtime.callFunctionOn", {
                objectId,
        ...
        // 模拟点击流程（对齐 DrissionPage）
        {
            ...
            try {
                ...
                const r = await this._ele.session.send("DOM.getNodeForLocation", {
                    x: checkX,
                    y: checkY,
                ...
                // 检查点击位置的元素是否是自己
                const elBackendId = this._ele.backendNodeId;
